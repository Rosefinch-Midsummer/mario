...
class Level():
    ...
    def setup_background(self):
        ...
        self.background= pygame.transform.scale(self.background, (int(rect.width*constants.BG_MULTI),int(rect.height*constants.BG_MULTI)))
    ...
    def is_frozen(self):
        return self.player.state in ['small2big', 'big2fire', 'big2small', 'fire2small']
    
    def update_player_position(self):
        # x direction
        self.player.rect.x += self.player.x_velo
        ...
        if not self.player.dead:
            self.player.rect.y += self.player.y_velo
            self.check_y_collisions()
    
    def check_x_collisions(self):
        check_group=pygame.sprite.Group(self.ground_items_group, self.brick_group, self.box_group)
        collided_sprite = pygame.sprite.spritecollideany(self.player, check_group)#字面意思：检查一个sprite与Spritegroup里任意一个对象有碰撞
        if collided_sprite:
    ...
    def check_y_collisions(self):
        ...
        if  ground_item:
            ...
            if self.player.y_velo<0:
                how='bumped'
            else:
                how='trampled'
                self.player.state='jump'
                self.player.rect.bottom=enemy.rect.top
                self.player.y_velo=self.player.jump_velo*0.8
    ...
    def update_game_window(self):
        ...
        if self.player.x_velo > 0 and self.player.rect.centerx > third and self.game_window.right<self.end_x: 
            self.game_window.x+=self.player.x_velo
