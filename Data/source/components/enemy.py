...
class Enemy(pygame.sprite.Sprite):
    def __init__(self , x ,y_bottom ,direction, name, frame_rects):
        ...
        self.timer=0
        self.x_velo=-1*constants.ENEMY_SPEED if self.direction == 0 else constants.ENEMY_SPEED
        self.y_velo=0
    ...
    def load_frames(self, frames_rects):
        for frame_rect in frames_rects:
            left_frame = tools.get_image(setup.GRAPHICS['enemies'], *frame_rect, (0,0,0), constants.EMEMY_MULTI)
    ...
    def go_die(self,how, direction=1):
        ...
        if how in ['bumped', 'slided']:
            self.x_velo=constants.ENEMY_SPEED*direction
            self.y_velo=-8
            self.gravity=0.6
            self.frame_index=2
            self.state='die'
...
class Koopa(Enemy):#乌龟怪类
    ...
    def trampled(self,level):
        ...
        if self.current_time - self.shell_timer >5000:
            self.x_velo = - constants.ENEMY_SPEED if self.direction==0 else constants.ENEMY_SPEED
